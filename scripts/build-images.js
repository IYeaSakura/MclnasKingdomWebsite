  ...
  if (priority === "high") {
    for (const width of sizes) {
      ...
      if (await shouldProcess(inputPath, avifPath)) {
        const avifResult = await convertImage(
          inputPath,
          avifPath,
          "avif",
          IMAGE_QUALITY.avif,
        ...
        if (avifResult.success) {
          const savings = (
            ((originalSize - avifResult.size) / originalSize) *
            100
          ).toFixed(1);
          console.log(
            `  ✓ AVIF ${width}px: ${(avifResult.size / 1024).toFixed(1)}KB (${savings}% smaller)`,
          );
        }
      } else {
      ...
      if (await shouldProcess(inputPath, webpPath)) {
        const webpResult = await convertImage(
          inputPath,
          webpPath,
          "webp",
          IMAGE_QUALITY.webp,
        ...
        if (webpResult.success) {
          const savings = (
            ((originalSize - webpResult.size) / originalSize) *
            100
          ).toFixed(1);
          console.log(
            `  ✓ WebP ${width}px: ${(webpResult.size / 1024).toFixed(1)}KB (${savings}% smaller)`,
          );
        }
      } else {
      ...
      if (width === sizes[0] && !isPng) {
        ...
        if (await shouldProcess(inputPath, jpegPath)) {
          const jpegResult = await convertImage(
            inputPath,
            jpegPath,
            "jpeg",
            IMAGE_QUALITY.jpeg,
          ...
          if (jpegResult.success) {
            const savings = (
              ((originalSize - jpegResult.size) / originalSize) *
              100
            ).toFixed(1);
            console.log(
              `  ✓ JPEG ${width}px: ${(jpegResult.size / 1024).toFixed(1)}KB (${savings}% smaller)`,
            );
          }
        } else {
    ...
    if (await shouldProcess(inputPath, avifPath)) {
      ...
      if (avifResult.success) {
        ...
        console.log(
          `  ✓ AVIF: ${(avifResult.size / 1024).toFixed(1)}KB (${savings}% smaller)`,
        );
      }
    } else {
    ...
    if (await shouldProcess(inputPath, webpPath)) {
      ...
      if (webpResult.success) {
        ...
        console.log(
          `  ✓ WebP: ${(webpResult.size / 1024).toFixed(1)}KB (${savings}% smaller)`,
        );
      }
    } else {
    ...
    if (!isPng) {
      ...
      if (await shouldProcess(inputPath, jpegPath)) {
        ...
        if (jpegResult.success) {
          ...
          console.log(
            `  ✓ JPEG: ${(jpegResult.size / 1024).toFixed(1)}KB (${savings}% smaller)`,
          );
        }
      } else {
